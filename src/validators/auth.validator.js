...
export const RegisterUserObject = z.object({
  ...
  address: z.object({
    ...
    latitude: z.string().refine(value => /^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?)$/.test(value), { message: 'Invalid latitude format' }),
    longitude: z.string().refine(value => /^[-+]?(180(\.0+)?|((1[0-7]\d)|(\d{1,2}))(\.\d+)?)$/.test(value), { message: 'Invalid longitude format' }),
  }),
});
